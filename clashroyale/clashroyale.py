...
class clashroyale:
    ...
    async def clashProfile(self, ctx, member: discord.Member = None):
        ...
        try:
            profiledata = requests.get(
                'https://api.royaleapi.com/player/{}?exclude=currentDeck,cards,battles,achievements'.format(profiletag),
        ...
        embed.set_thumbnail(url="https://royaleapi.github.io/cr-api-assets/arenas/{}.png".format(
            profiledata['arena']['arena'].replace(' ', '').lower()))
    ...
    async def chests(self, ctx, member: discord.Member = None):
        ...
        try:
            profiledata = requests.get('https://api.royaleapi.com/player/{}/chests'.format(profiletag),
                                       headers=self.getAuth(), timeout=10).json()
        except:
            await self.bot.say("Error: cannot reach Clash Royale Servers. Please try again later.")
            return

        valuechestText = ' '.join(profiledata['upcoming']).replace('silver', '<:silver:385784583343439882>').replace(
            'gold', '<:gold:385784630227369985>').replace('giant', '<:giant:380832560504373249>').replace('epic',
                                                                                                          '<:epic:380832620059033610>').replace(
            'super magical', '<:super:380832745305276416>').replace('magical', '<:magic:380832656704798731>').replace(
            'legendary', '<:legend:380832458482122752>')
    ...
    async def clashDeck(self, ctx, member: discord.Member = None):
        ...
        try:
            ...
            try:
                profiledata = requests.get('https://api.royaleapi.com/player/{}?keys=deckLink'.format(profiletag),
                                           headers=self.getAuth(), timeout=10).json()
    ...
    async def clan(self, ctx, clantag):
        ...
        try:
            clandata = requests.get('https://api.royaleapi.com/clan/{}'.format(clantag), headers=self.getAuth(),
                                    timeout=10).json()

            embed = discord.Embed(title=clandata['name'] + " (#" + clandata['tag'] + ")",
                                  url="https://legendclans.com/clanInfo/{}".format(clandata['tag']),
                                  description=clandata['description'], color=0xFAA61A)
