...
class legend:
    ...
    async def clans_pb(self, ctx, clankey, pb: int):
        ...
        try:
            ...
            await self.bot.say(
                "Please use a valid clanname : " + ", ".join(key for key in self.c.keys())
            )
            return
    ...
    async def _is_member(self, member):
        server = member.server
        botcommander_roles = [
            discord.utils.get(server.roles, name=r)
            for r in ["Member", "Co-Leader", "Hub Officer", "Clan Deputy", "Clan Manager"]
        ]
        botcommander_roles = set(botcommander_roles)
        author_roles = set(member.roles)
        if len(author_roles.intersection(botcommander_roles)):
    ...
    async def clanwarReadiness(self, cards):
        ...
        readiness = {}
        leagueLevels = {"legend": 12, "gold": 11, "silver": 10, "bronze": 9}
    ...
    async def legend(self, ctx, member: discord.Member = None):
        ...
        if member is None:
            ...
            try:
                ...
                return await self.bot.say(
                    "You must assosiate a tag with this member first using ``{}save #tag @member``".format(
                        ctx.prefix
                    )
                )
        ...
        clandata = sorted(
            clandata, key=lambda x: (x.required_trophies, x.clan_score), reverse=True
        )

        embed = discord.Embed(color=0xFAA61A)
        if "url" in self.settings and "family" in self.settings:
            embed.set_author(
                name=self.settings["family"],
        ...
        for clan in clandata:
            ...
            if cwr > 0:
                ...
                if member is not None:
                    plyrLeagueCWR = await self.getBestPerc(
                        cards, await self.getLeague(warTrophies)
                    )

            if bonustitle is not None:
                title += bonustitle

            desc = "{} {}  <:crtrophy:448609948008579073> " "{}+  {} {}".format(
                emoji,
                showMembers,
            ...
            if (
                (member is None)
                or (
                    (clan.required_trophies <= trophies)
                    and (maxtrophies > personalbest)
                    and (plyrLeagueCWR >= cwr)
                    and (trophies - clan.required_trophies < 1200)
                    and (clan.type != "closed")
                )
                or (
                    (clan.required_trophies < 2000)
                    and (member_count != 50)
                    and (2000 < trophies < 4000)
                    and (clan.type != "closed")
                )
            ):
                foundClan = True
    ...
    async def approve(self, ctx, member: discord.Member, clankey):
        ...
        try:
            ...
            return await self.bot.say(
                "Please use a valid clanname: {}".format(await self.clans.namesClans())
            )
        ...
        if membership:
            ...
            if await self.clans.numWaiting(clankey) > 0:
                if await self.clans.checkWaitingMember(clankey, member.id):
                    ...
                    if await self.clans.getWaitingIndex(clankey, member.id) > canWait:
                        return await self.bot.say(
                            "Approval failed, you are not first in queue for the waiting list on this server."
                        )
                    ...
                    return await self.bot.say(
                        "Approval failed, there is a waiting queue for this clan. Please first join the waiting list."
                    )
            ...
            try:
                ...
                await self.bot.send_message(
                    discord.Object(id="375839851955748874"), content=roleName.mention, embed=embed
                )
            except discord.errors.Forbidden:
                await self.bot.say(
                    "Approval failed, {} please fix your privacy settings, we are unable to send you Direct Messages.".format(
                        member.mention
                    )
                )
        else:
    ...
    async def newmember(self, ctx, member: discord.Member):
        ...
        if not allowed:
            return await self.bot.say(
                "You dont have enough permissions to use this command on others."
            )
        ...
        if membership:
            ...
            try:
                ...
                return await self.bot.say(
                    (
                        "Membership failed, {} please fix your privacy settings, "
            ...
            try:
                ...
                mymessage += (
                    "**"
                    + await self.clans.getClanData(savekey, "role")
                    + "** and **Member** roles added."
                )
            except discord.Forbidden:
                await self.bot.say(
            ...
            await self.bot.send_message(
                discord.Object(id="374596069989810178"), welcomeMsg.format(member, server)
            )
            ...
            await self.bot.send_message(
                discord.Object(id="375839851955748874"),
                "**{}** recruited **{} (#{})** to {}".format(
                    ctx.message.author.display_name, ign, profiletag, roleName.mention
                ),
            )
    ...
    async def waiting(self, ctx, member: discord.Member, clankey):
        ...
        roleName = discord.utils.get(
            server.roles, name=await self.clans.getClanData(clankey, "role")
        )
        await self.bot.send_message(
            discord.Object(id="375839851955748874"),
            "**{} (#{})** added to the waiting list for {}".format(
                ign, profiletag, roleName.mention
            ),
        )
    ...
    async def remove(self, ctx, member: discord.Member, clankey):
        ...
        try:
            ...
            await self.bot.say(
                member.mention
                + " has been removed from the waiting list for **"
                + clan_name
                + "**."
            )
        except ValueError:
    ...
    async def waitinglist(self, ctx):
        ...
        for clan in self.clans.keysClans():
            if await self.clans.numWaiting(clan) > 0:
                ...
                embed.add_field(
                    name=await self.clans.getClanData(clan, "name"), value=message, inline=False
                )
        ...
        if not message:
            ...
            embed.set_author(
                name="Legend Family Waiting List", icon_url="https://i.imgur.com/dtSMITE.jpg"
            )
            embed.set_footer(text=credits, icon_url=creditIcon)
            await self.bot.say(embed=embed)
    ...
    async def audit(self, ctx, clankey):
        ...
        for d_member in d_members:
            try:
                ...
                try:
                    if self.seen[ctx.server.id][d_member.id]["TIMESTAMP"] < time.time() - 691200:
        ...
        for index, player_tag in enumerate(cr_members_tag):
            try:
                ...
                if (cr_members_name[index] not in dc_member.display_name) or (
                    clan_nickname not in dc_member.display_name
                ):
        ...
        cr_clanSettings.append(
            "Legend Family🔥14 Clans🔥LegendClans.com🔥Events & Prizes🔥Apply at legendclans.com/discord🔥"
            in clandata.description
        )
        ...
        if False in cr_clanSettings:
            ...
            if not cr_clanSettings[0]:
                message += "\n• Clan Badge is incorrect."
            if not cr_clanSettings[1]:
                message += "\n• Clan Location is incorrect."
            if not cr_clanSettings[2]:
                message += "\n• Clan description is incorrect."
            if not cr_clanSettings[3]:
                message += "\n• Clan is closed."

            message += "```\n\n"

        if cr_members_with_no_player_tag:
            message += ":warning: **({})** Players in **{}**, but have **NOT** joined discord: ```• ".format(
                len(cr_members_with_no_player_tag), clan_name
            )
            message += "\n• ".join(cr_members_with_no_player_tag)
            message += "```\n\n"

        if d_members_with_no_player_tag:
            message += ":warning: **({})** Players with **{}** role, but have **NO** tags saved: ```• ".format(
                len(d_members_with_no_player_tag), clan_name
            )
            message += "\n• ".join(d_members_with_no_player_tag)
            message += "```\n\n"

        if d_members_not_in_clan:
            message += ":warning: **({})** Players with **{}** role, but have **NOT** joined the clan: ```• ".format(
                len(d_members_not_in_clan), clan_name
            )
            message += "\n• ".join(d_members_not_in_clan)
            message += "```\n\n"

        if d_members_without_role:
            message += ":warning: **({})** Players in **{}**, but **DO NOT** have the clan role: ```• ".format(
                len(d_members_without_role), clan_name
            )
            message += "\n• ".join(d_members_without_role)
            message += "```\n\n"

        if d_members_without_name:
            message += ":warning: **({})** Players in **{}**, but have an **INCORRECT** nickname: ```• ".format(
                len(d_members_without_name), clan_name
            )
        ...
        if cr_members_with_less_trophies:
            message += ":warning: **({})** Players in **{}**, but **DO NOT** meet the trophy requirements: ```• ".format(
                len(cr_members_with_less_trophies), clan_name
            )
            message += "\n• ".join(cr_members_with_less_trophies)
            message += "```\n\n"

        if d_members_inactive:
            message += ":warning: **({})** Players in **{}**, but **NOT** active on Discord: ```• ".format(
                len(d_members_inactive), clan_name
            )
    ...
    async def topmembers_trophies(self, role: str = None):
        ...
        if role not in ["leader", "coleader", "elder", "member", None]:
            return await self.bot.say(
                "Invalid role! Please chose between: leader, coleader, and elder."
            )

        embed = discord.Embed(color=0xFAA61A)
        embed.set_author(name=title, icon_url="https://i.imgur.com/dtSMITE.jpg")
        ...
        try:
            if "url" in self.settings:
                ...
                allplayers = requests.get(
                    "http://royaleapi.com/clan/family/legend/members/datatable", timeout=15
                ).json()
        except:
            return await self.bot.say(
                "Error: cannot reach Clash Royale Servers. Please try again later."
    ...
    async def topclans(self):
        ...
        embed.set_author(
            name="Local International Leaderboard",
    ...
    async def platoon(self, ctx, member: discord.Member):
        ...
        try:
            if role in member.roles:
                await self.bot.remove_roles(member, role)
                await self.bot.say(
                    "{} Role Removed from {}".format(role.name, member.display_name)
                )
            else:
                await self.bot.add_roles(member, role)
    ...
    async def guest(self, ctx, member: discord.Member):
        ...
        try:
            ...
            return await self.bot.say(
                "Command failed, {} please fix your privacy settings, we are unable to send you Guest Rules.".format(
                    member.mention
                )
            )
    ...
    async def _reactrole_embed(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="React with emojis to choose which notifications you would like to turn on!",
            color=0x0080FF,
        )
        embed.add_field(
            name="Challenges",
            value=f":hourglass: — When a Word Challenge Game starts in {server.get_channel('405062108175269898').mention}",
            inline=False,
        )
        embed.add_field(
            name="Heist",
            value=f":bank: — When a Heist is planned in {server.get_channel('381338682298466315').mention}",
            inline=False,
        )
        embed.add_field(
            name="Duels",
            value=f"<:battle:466243249149837314> — When a player is looking for a Clash Royale battle in {server.get_channel('430083671429611530').mention}",
            inline=False,
        )
        embed.add_field(
            name="Giveaways",
            value=f"<:coin:380832316932489268> — When a giveaway for credits/games is hosted in {server.get_channel('421623464567504896').mention}",
            inline=False,
        )
        embed.add_field(
            name="Tournaments",
            value=f"<:tourney:466243255504470036> — When a 100+ player tournament is hosted in {server.get_channel('374597050530136064').mention}",
            inline=False,
        )
        embed.add_field(
            name="Race",
            value=f":race_car: — When a CR Race starts in {server.get_channel('490125656223514627').mention}",
            inline=False,
        )
        embed.set_footer(text=credits, icon_url=creditIcon)

        embed2 = await self.bot.get_message(
            server.get_channel("426697128883257344"), "477582487162454056"
        )
    ...
    async def cwstats(self, ctx, tag):
        ...
        if not await self.tags.verifyTag(tag):
            return await self.bot.say(
                "The ID you provided has invalid characters. Please try again."
            )
        ...
        message = "\n**{}**```{}\t{}\t{}\n".format(
            tourney.name, "CLAN".ljust(17), "SCORE".ljust(9), "PARTICIPANTS"
        )
        clanwar_dict = OrderedDict(
            sorted(clanwar_dict.items(), key=lambda x: x[1]["score"], reverse=True)
        )
        for x in clanwar_dict:
            message += "{}\t{}\t{}\n".format(
                x.ljust(17),
                str(clanwar_dict[x]["score"]).ljust(9),
                clanwar_dict[x]["participants"],
            )
        message += "```"
        await self.bot.say(message)
